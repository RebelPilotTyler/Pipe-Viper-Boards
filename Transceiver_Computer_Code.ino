//Transceiver Computer Code, a program for testing the transceivers for Pipe Viper. This code is for the anchor only.
...
void setup() {
  ...
  Serial.println(F("Pipe Viper Anchor"));
...
void loop() {
  if (role) {
    ...
    //while (!Serial.available()) { //While loop for serial comms removed favoring button input
      //wait for user input
    //}
    //payload = Serial.read();
